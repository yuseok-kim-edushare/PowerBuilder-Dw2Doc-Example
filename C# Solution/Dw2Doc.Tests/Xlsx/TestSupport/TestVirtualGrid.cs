...
namespace Appeon.DotnetDemo.Dw2Doc.Tests.Xlsx.TestSupport
{
    /// <summary>
    ...
    public static class TestVirtualGridFactory
    {
        ...
        private static void SetAttributes(VirtualGrid grid, Dictionary<string, DwObjectAttributesBase> attributes)
        {
            ...
            if (controlAttributesField != null)
            {
                ...
                Console.WriteLine($"Applied {attributes.Count} attributes directly to VirtualGrid");
                return;
            }
            
            // Try the method approach as a fallback
            ...
            if (method != null)
            {
                ...
                Console.WriteLine($"Applied {attributes.Count} attributes via method");
                return;
            }
            
            // Final fallback - try to apply attributes to each cell
            bool appliedAny = false;
            foreach (var row in grid.Rows)
            {
                foreach (var cell in row.Objects)
                {
                    if (attributes.TryGetValue(cell.Object.Name, out var attr))
                    {
                        // Try to set the attribute directly on the cell
                        var cellAttrsField = typeof(VirtualCell).GetField("_attributes", 
                            System.Reflection.BindingFlags.NonPublic | 
                            System.Reflection.BindingFlags.Instance);
                        
                        if (cellAttrsField != null)
                        {
                            cellAttrsField.SetValue(cell, attr);
                            appliedAny = true;
                        }
                    }
                }
            ...
            if (appliedAny)
            {
                Console.WriteLine("Applied attributes directly to cells");
            }
            else
            {
                Console.WriteLine("WARNING: Could not apply attributes to VirtualGrid!");
            }
        }
    }
} 